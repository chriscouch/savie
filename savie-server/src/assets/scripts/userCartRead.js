...
userCartRead.preloadData = async () => {
  // Load catalog items.
  const {statusCode, responsePayload: catalogItems} = await httpClient.request({path: '/api/catalogs', method: 'GET'});

  // Quit if there are no catalog items or status code isn't 200.
  if (!catalogItems || statusCode !== 200) {
    return false;
  }
  ...
  if (shoppingCart.cart.items && shoppingCart.cart.items.length) {
    ...
    shoppingCart.cart.items.forEach((cartItem) => {
      const cartItemObject = catalogItems.filter(catalogItem => catalogItem.id === cartItem.id).pop();
