...
const templateLoader = async (templateName, templateData = {}) => {
  ...
  const templatesDir = path.join(__dirname, `/../${config.templateDir}/`)
  ...
  // If the templateName starts with a '_' the template doesn't require the templateWrapper.
  
  if (templateName.search(/^_/) == -1 ){
  ...
  // Embed requested template into template wrapper.
    compiledTemplate = templateWrapperString.replace('{{pageContent}}', templateString)
  } else {
    if (!templateString) {
      return false
    }
