/**
 * @module
 * @fileoverview Primary server related tasks.
 */
...
// All the server logic for both HTTP and HTTPS servers.
  ...
  init : () => {
    ...
    server.httpServer.listen(config.httpPort, () => {
      console.log("\x1b[36m%s\x1b[0m",`HTTP server is listening on ${config.httpPort}`)
    })

    //Start the server and listen on port in 'config'
    server.httpsServer.listen(config.httpsPort, () => {
      console.log("\x1b[36m%s\x1b[0m", `HTTPS server is listening on ${config.httpsPort}.`)
    })
  }
} // server close.
// Export the module.
